...
const defaultLoc = { name: 'Negba 7', city: 'Tel Aviv', country: 'Israel', longitude: 34.781769, latitude: 32.085300 };
...
export const fetchFocus = async (): Promise<any> => {
    ...
    let y = await Location.reverseGeocodeAsync(x.coords);

    return { name: y[0].name, city: y[0].city, country: y[0].country, longitude: x.coords.longitude, latitude: x.coords.latitude };
}
...
export const initMap = (): any => async (dispatch: any): Promise<void> => {
    ...
    dispatch({
        type: SET_STATE,
        payload: { data: { pins: pins ? pins : [], focus: focus ? focus : defaultLoc } }
    });
}
...
export const initFocus = (): any => async (dispatch: any): Promise<void> => {
    ...
    dispatch({
        type: SET_FOCUS,
        payload: { data: focus ? focus : defaultLoc }
    });
}
...
export const removeMarker = (markerId: string, markers: any): any => async (dispatch: any): Promise<void> => {
    try {
        const res: any = await Axios.delete(`${ENDPOINT}/pin/${markerId}`, {
            headers: {
                'API-KEY': API_KEY
            }
        });
