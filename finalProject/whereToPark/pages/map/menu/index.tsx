...
import { fetchFocus, refreshMarkers, setFocus } from '../actions';
...
const Menu = ({ navigation }: any): any => {
  ...
  const handleInsert = async (): Promise<void> => {
    try {
      ...
      await Axios.post(`${ENDPOINT}/pin/`, data, {
        headers: {
          'API-KEY': API_KEY
        }
      });
  ...
  return (
    <React.Fragment>
      <View style={{ ...styles.btn, ...styles.favBtn }}>
        <Button
          icon={<Entypo name="menu" size={40} color="#fff" />}
          type="clear"
          onPress={() => {
      ...
      <View style={{ ...styles.btn, ...styles.addBtn }}>
        <Button
          icon={<Entypo name="plus" size={40} color="#fff" />}
          type="clear"
      ...
      <View style={{ ...styles.btn, ...styles.locationBtn }}>
        <Button
          icon={<MaterialIcon name="my-location" size={40} color="#fff" />}
          type="clear"
