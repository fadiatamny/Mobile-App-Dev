...
const ModalView = ({ modalVisible, setModalVisible, data, location }: any): any => {
    ...
    let pins = useSelector((state: any) => state.user.favs);
    const alreadyFav: boolean = exists({ longitude: location.longitude, latitude: location.latitude }, pins);

    const openMaps = async (): Promise<void> => {
        const scheme = Platform.OS === 'ios' ? 'http://maps.apple.com/?daddr=' : 'https://www.google.com/maps/search/?api=1&query=';
                ...
                >
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            ...
                            <Text style={styles.modalSubText}>{data[0].city} - {data[0].country}</Text>
                            {!alreadyFav ?
                                <Button
